...
import { DrawOptions, IExtendedRouteButtonElement } from "../types/EditorTypes";
  ...
  implements IExtendedRouteButtonElement
{
  ...
  override draw(ctx: CanvasRenderingContext2D, options?: DrawOptions): void {
    ...
    ctx.fillText(
      this.label.trim().length > 0 ? this.label : "AUTO",
      this.centerX,
      this.centerY + 10,
    );
    ctx.restore();
  ...
  static fromJSON(data: IExtendedRouteButtonElement): ExtendedRouteButtonElement {

...
import type { DrawOptions, IAudioListButtonElement } from "../types/EditorTypes";
...
export class AudioListButtonElement
  extends ClickableBaseElement
  implements IAudioListButtonElement
{
  override type: typeof ELEMENT_TYPES.BUTTON_AUDIO_LIST = ELEMENT_TYPES.BUTTON_AUDIO_LIST;
  label = "Audio list";
  ...
  playItem(item: AudioListButtonItem, onChanged?: () => void): void {
    if (!item.fileName) {
      showWarningMessage(i18n.t("common.warning"), i18n.t("audio.messages.fileNameMissing"));
      return;
    }
    ...
    audioManager.play(item.fileName, {
      ...
      onError: (error) => {
        clearActivePlayback();
        showErrorMessage(i18n.t("common.error"), errorToString(error));
      },
    });
  }
  ...
  private stylePlayButton(play: HTMLButtonElement, active: boolean): void {
    ...
    play.style.color = active ? "white" : "inherit";
  }
  private updateRuntimePopupActiveStates(): void {
    ...
    const buttons = popup.querySelectorAll<HTMLButtonElement>(AUDIO_LIST_PLAY_BUTTON_SELECTOR);
    buttons.forEach((button) => {
      this.stylePlayButton(button, button.dataset.audioListPlayId === this.activeItemId);
    });
  }
  ...
  override draw(ctx: CanvasRenderingContext2D, options?: DrawOptions): void {
    ...
    ctx.scale(
      (Math.min(this.width, this.height) - 16) / 24,
      (Math.min(this.width, this.height) - 16) / 24,
    );
    ...
    if (this.label) {
      ...
      ctx.fillText(this.label, this.posLeft + this.width / 2, this.posBottom - 5);
      ctx.restore();
    }
    this.endDraw(ctx);
  ...
  static fromJSON(data: IAudioListButtonElement): AudioListButtonElement {
    const e = new AudioListButtonElement(data.x, data.y);
    e.id = data.id;
    e.name = data.name;
    ...
    e.audioItems = Array.isArray(data.audioItems)
      ? data.audioItems
          ...
          .map((item: any) => ({
            ...
            name: typeof item.name === "string" ? item.name : "Audio",
            fileName: typeof item.fileName === "string" ? item.fileName : "",
          }))
      : [];
    return e;
  }
  override clone(): AudioListButtonElement {
    const copy = new AudioListButtonElement(this.x, this.y);
    copy.id = generateId();

...
import type { DrawOptions, IButtonScriptElement } from "../types/EditorTypes";
...
export class ButtonScriptElement extends BaseElement implements IButtonScriptElement {
  ...
  static fromJSON(data: IButtonScriptElement): ButtonScriptElement {
    const element = new ButtonScriptElement(data.x, data.y);
    element.id = data.id;
    ...
    element.bg = data.bg;
    element.fg = data.fg;
    element.colorOn = data.colorOn || "#7048e8";
    element.colorOff = data.colorOff || "#5f3dc4";
    element.textOn = data.textOn || "JS";
    element.textOff = data.textOff || "JS";
    element.script = typeof data.script === "string" ? data.script : DEFAULT_SCRIPT;
    return element;
  }

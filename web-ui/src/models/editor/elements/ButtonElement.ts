...
const buttonInstances = new Set<ButtonElement>();
export class ButtonElement extends ClickableBaseElement implements IButtonElement {
  ...
  sendConfiguredState(logicalOn: boolean): boolean {
    if (!Number.isInteger(this.address) || this.address < 1 || this.address > 2048) {
      return false;
    }
    ...
    const physicalValue = logicalOn ? this.activeValue : this.offValue;
    const sent = wsApi.setBasicAccessory(this.address, physicalValue);
    return sent;
  }
  ...
  override mouseDown(_event: MouseEvent): void {
    ...
    if (this.behavior === "momentary") {
      ...
      this.momentaryPressed = this.sendConfiguredState(true);
      return;
    }
  ...
  override mouseUp(_event: MouseEvent): void {
    if (this.behavior !== "momentary" || !this.momentaryPressed) {
      return;
    }
  ...
  override toJSON(): IButtonElement {
    return {
      ...
      outputMode: this.outputMode === "extended" ? "extended" : "accessory",
      behavior: this.behavior,
      pulseDurationMs: this.pulseDurationMs,
      address: this.address,
      activeValue: this.activeValue,
      offValue: this.offValue === this.activeValue ? !this.activeValue : this.offValue,
  ...
  static fromJSON(data: IButtonElement): ButtonElement {
    ...
    e.outputMode = data.outputMode === "extended" ? "extended" : "accessory";
    e.behavior =
      data.behavior === "push" || data.behavior === "momentary" ? data.behavior : "toggle";
     ...
     */
    e.offValue = data.offValue ?? !e.activeValue;
  ...
  override getEditableProperties(): IEditableProperty[] {
    return [
      ...
      /*
       * TurnoutBitPropertyEditor recognizes ButtonElement and renders
...
/**
 * Keep every ButtonElement instance synchronized with DCC-EX extended
 * accessory feedback.
 *
...
wsClient.on("signalAspectChanged", (data) => {
  for (const button of buttonInstances) {
    if (button.outputMode !== "extended" || button.address !== data.address) {
      continue;
    }

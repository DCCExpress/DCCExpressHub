...
public sealed class SignalAutomationEngine
{
    ...
    sealed class Condition { public Source Source; public ushort Address; public bool Value; }
    sealed class Rule { public int Value; public List<Condition> Conditions=[]; }
    sealed class SignalRuleSet
    {
        public ushort Address; public bool Extended=true; public byte Outputs=1;
    ...
    {
        if(type is "turnoutChanged" or "sensorChanged") _=EvaluateAsync();
    }
    ...
    bool Parse(string text,out bool enabled,out List<SignalRuleSet> signals)
    {
        ...
        foreach(var raw in text.Split('\n')){
            ...
            JsonDocument doc;try{doc=JsonDocument.Parse(line);}catch{continue;}
            using(doc){
                ...
                if(kind!="signal"||!ParseSignal(o,out var sig))continue;
    ...
    bool ParseSignal(JsonElement row,out SignalRuleSet signal)
    {
        ...
        if(address is <=0 or >65535)return false;

        // Address is the only persisted/runtime identity. "id" is deliberately ignored.
        ...
        int def=GetInt(row,"default");if(!Fits(extended,outputs,def))return false;
        if(!row.TryGetProperty("rules",out var rules)||rules.ValueKind!=JsonValueKind.Array)return false;

        signal=new(){Address=(ushort)address,Extended=extended,Outputs=outputs,DefaultValue=def};
        ...
        foreach(var rr in rules.EnumerateArray()){
            ...
            foreach(var c in cs.EnumerateArray()){
                if(c.ValueKind!=JsonValueKind.Array){valid=false;break;}
                var a=c.EnumerateArray().ToArray();
                string source=a.Length>0&&a[0].ValueKind==JsonValueKind.String?a[0].GetString()??"":"";

                // v4: ["turnout",address,physicalValue], ["sensor",address,value]
                if(a.Length==3){
                    int addr=AsInt(a[1]),physical=AsInt(a[2]);
                    if(addr<=0||addr>65535||(physical!=0&&physical!=1)){valid=false;break;}
                    if(source=="turnout"){
                        var t=_runtime.FindAccessory(RuntimeAccessoryKind.Turnout,(ushort)addr);
                        if(t==null){valid=false;break;}
                        rule.Conditions.Add(new(){Source=Source.Turnout,Address=(ushort)addr,Value=(physical!=0)==t.ClosedValue});
                    }else if(source=="sensor"){
                        if(_runtime.FindSensor((ushort)addr)==null){valid=false;break;}
                        rule.Conditions.Add(new(){Source=Source.Sensor,Address=(ushort)addr,Value=physical!=0});
                    }else{valid=false;break;}
                    continue;
                }

                // Read-only legacy migration support. Never used as runtime identity.
                if(a.Length==4){
                    int legacyId=AsInt(a[1]),channel=AsInt(a[2]),val=AsInt(a[3]);
                    if(legacyId<=0||legacyId>65535||channel<0||channel>1||(val!=0&&val!=1)){valid=false;break;}
                    if(source=="turnout"){
                        var t=_runtime.FindAccessoryById(RuntimeAccessoryKind.Turnout,(ushort)legacyId,(byte)channel);
                        if(t==null){valid=false;break;}
                        rule.Conditions.Add(new(){Source=Source.Turnout,Address=t.Address,Value=val!=0});
                    }else if(source=="sensor"){
                        var s=_runtime.FindSensorById((ushort)legacyId);
                        if(s==null){valid=false;break;}
                        rule.Conditions.Add(new(){Source=Source.Sensor,Address=s.Address,Value=val!=0});
                    }else{valid=false;break;}
                    continue;
                }
                valid=false;break;
    ...
    bool Matches(Condition c)
    {
        ...
        var t=_runtime.FindAccessory(RuntimeAccessoryKind.Turnout,c.Address);return t!=null&&t.Closed==c.Value;
    }
    ...
    async Task ApplyAsync(SignalRuleSet s,int value)
    {
        ...
        if(target==null){Console.WriteLine($"SignalAutomation: target address {s.Address} not found");return;}
        if(s.HasAppliedValue&&s.AppliedValue==value)return;

        if(s.Extended){
            if(!await _cc.SetSignalAspectAsync(s.Address,value))return;
            _runtime.SetSignal(s.Address,value);
        }else{
            for(byte i=0;i<s.Outputs;i++){
                bool active=((value>>i)&1)!=0;
                if(!await _cc.SetAccessoryAsync((ushort)(s.Address+i),active))return;
                _runtime.SetAccessory((ushort)(s.Address+i),active);
            }
            _runtime.SetSignal(s.Address,value);
        }

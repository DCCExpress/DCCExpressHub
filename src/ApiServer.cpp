...
const char* ApiServer::mimeFor(
  ...
  if (path.endsWith(".ndjson")) return "application/x-ndjson; charset=utf-8";
...
void ApiServer::sendFsFile(
  ...
  if (LittleFS.exists(gz)) {
    auto* response = request->beginResponse(LittleFS, gz, mimeFor(path), false);
    response->addHeader("Content-Encoding", "gzip");
    response->addHeader("Cache-Control", cacheControlFor(path));
    request->send(response);
    return;
  }

  if (LittleFS.exists(path)) {
    auto* response = request->beginResponse(LittleFS, path, mimeFor(path), false);
    response->addHeader("Cache-Control", cacheControlFor(path));
    request->send(response);
    return;
  }
...
void ApiServer::handleLayoutBody(
    ...
    size_t total) {
  if (index == 0) {
    _layoutUpload.begin(_files, LAYOUT_PATH, total);
  }

  if (!_layoutUpload.failed()) {
    _layoutUpload.write(data, len);
  }
  ...
  if (!_layoutUpload.finish()) {
    _layoutUpload.abort();
    response["ok"] = false;
    response["message"] = "Layout upload failed";
  ...
  Logger::info(
      ...
      " sensors; signal automation reload=" +
      String(signalAutomationReloaded ? "ok" : "failed"));
...
bool ApiServer::verifyLocosTemp() {
  const String tempPath = _locosUpload.tempPath();
  File file = _files.openRead(tempPath.c_str());
...
void ApiServer::handleLocosBody(
    ...
    size_t total) {
  if (index == 0) {
    _locosUpload.begin(_files, LOCOS_PATH, total);
  }
  if (!_locosUpload.failed()) _locosUpload.write(data, len);
  if (index + len != total) return;

  JsonDocument response;
  if (!_locosUpload.finish()) {
    _locosUpload.abort();
    response["ok"] = false;
    response["message"] = "Locomotive upload failed";
    sendJson(request, 507, response);
    return;
  }
  if (!verifyLocosTemp()) {
    _locosUpload.abort();
    response["ok"] = false;
    response["message"] = "Expected locomotive JSON array";
    sendJson(request, 400, response);
    return;
  }
  if (!_locosUpload.commit()) {
    response["ok"] = false;
    response["message"] = "Locomotive atomic rename failed";
    sendJson(request, 500, response);
    return;
  }
  if (_onLocomotivesSaved && !_onLocomotivesSaved()) {
    response["ok"] = false;
    response["message"] = "Locomotive configuration committed but runtime reload failed";
    sendJson(request, 500, response);
    return;
  }
...
void ApiServer::handleSignalLogicBody(
    ...
    size_t total) {
  if (index == 0) {
    _signalLogicUpload.begin(_files, SIGNAL_LOGIC_PATH, total);
  }
  if (!_signalLogicUpload.failed()) _signalLogicUpload.write(data, len);
  if (index + len != total) return;

  JsonDocument response;
  if (!_signalLogicUpload.finish()) {
    _signalLogicUpload.abort();
    response["ok"] = false;
    response["message"] = "Signal automation upload failed";
    sendJson(request, 507, response);
    return;
  }
  if (!verifySignalLogicTemp()) {
    _signalLogicUpload.abort();
    response["ok"] = false;
    response["message"] = "Invalid signal automation NDJSON v2";
    sendJson(request, 400, response);
    return;
  }
  if (!_signalLogicUpload.commit()) {
    response["ok"] = false;
    response["message"] = "Signal automation atomic rename failed";
    sendJson(request, 500, response);
    return;
  }
  if (!_signalAutomation.reload()) {
    response["ok"] = false;
    response["message"] = "Signal automation committed but runtime reload failed";
    sendJson(request, 500, response);
    return;
  }

  _signalAutomation.evaluate();
  Logger::info("Signal automation saved and reloaded: " + String(total) + " bytes");
  response["ok"] = true;
  response["bytes"] = total;
  sendJson(request, 200, response);
}
...
void ApiServer::setupApi() {
  ...
  _server.on(
      ...
      [this](AsyncWebServerRequest* request) {
        ...
        capabilities["currentTelemetry"] = CommandCenterCapabilities::currentTelemetry();
        capabilities["trackConfiguration"] = CommandCenterCapabilities::trackConfiguration();
        capabilities["locomotiveControl"] = CommandCenterCapabilities::locomotiveControl();
        capabilities["locomotiveFunctions"] = CommandCenterCapabilities::locomotiveFunctions();
        capabilities["turnoutControl"] = CommandCenterCapabilities::turnoutControl();
        capabilities["basicAccessory"] = CommandCenterCapabilities::basicAccessory();
        capabilities["signalAspect"] = CommandCenterCapabilities::signalAspect();
  ...
  _server.on(
      ...
      [this](AsyncWebServerRequest* request) {
        ...
        if (!parseEndpointFromRequest(request, host, port, error)) {
          doc["ok"] = false;
          doc["message"] = error;
        ...
        String powerText;
        bool powerIncludesProgramming = _wsProtocol.powerIncludesProgramming();
        if (!readPostValue(request, "powerIncludesProgramming", powerText) ||
            !parseBooleanValue(powerText, powerIncludesProgramming)) {
          doc["ok"] = false;
          doc["message"] = "Invalid powerIncludesProgramming value";
          sendJson(request, 400, doc);
          return;
        }
  ...
  _server.on(
      ...
      [](AsyncWebServerRequest* request) {
        ...
        const CommandCenterProbeResult probe = probeDccExEndpoint(host, port);
        doc["ok"] = probe.dccExAlive;
        doc["tcpConnected"] = probe.tcpConnected;
        doc["dccExAlive"] = probe.dccExAlive;
        doc["reply"] = probe.reply;
        doc["elapsedMs"] = probe.elapsedMs;
        if (!probe.tcpConnected) doc["message"] = "Command center connection failed";
        else if (!probe.dccExAlive) doc["message"] = "Command center did not answer";
        sendJson(request, probe.dccExAlive ? 200 : 502, doc);
      });
  ...
  _server.on(
      ...
      [this](AsyncWebServerRequest* request) {
        ...
        doc["accessories"] = _runtime.accessoryCount();
        doc["sensors"] = _runtime.sensorCount();
        sendJson(request, 200, doc);
      });
  ...
  _server.on(
      ...
      HTTP_GET,
      [this](AsyncWebServerRequest* request) {
        if (!_files.exists(LAYOUT_PATH)) {
          auto* response = request->beginResponse(200, "application/json", "{}");
          response->addHeader("Cache-Control", "no-store");
          request->send(response);
          return;
        }
        auto* response = request->beginResponse(LittleFS, LAYOUT_PATH, "application/json", false);
        response->addHeader("Cache-Control", "no-store");
        request->send(response);
      });
  ...
  _server.on(
      ...
      [this](AsyncWebServerRequest* request, uint8_t* data, size_t len, size_t index, size_t total) {
        handleLayoutBody(request, data, len, index, total);
      });
  ...
  _server.on(
      ...
      HTTP_GET,
      [this](AsyncWebServerRequest* request) {
        if (!_files.exists(LOCOS_PATH)) {
          auto* response = request->beginResponse(200, "application/json", "[]");
          response->addHeader("Cache-Control", "no-store");
          request->send(response);
          return;
        }
        auto* response = request->beginResponse(LittleFS, LOCOS_PATH, "application/json", false);
        response->addHeader("Cache-Control", "no-store");
        request->send(response);
      });
  ...
  _server.on(
      ...
      HTTP_GET,
      [this](AsyncWebServerRequest* request) {
        if (!_files.exists(SIGNAL_LOGIC_PATH)) {
          auto* response = request->beginResponse(404, "text/plain; charset=utf-8", "Not found");
          response->addHeader("Cache-Control", "no-store");
          request->send(response);
          return;
        }
        auto* response = request->beginResponse(LittleFS, SIGNAL_LOGIC_PATH, "application/x-ndjson; charset=utf-8", false);
        response->addHeader("Cache-Control", "no-store");
        request->send(response);
      });
  ...
  _server.on(
      ...
      nullptr,
      [this](AsyncWebServerRequest* request, uint8_t* data, size_t len, size_t index, size_t total) {
        handleSignalLogicBody(request, data, len, index, total);
      });
  ...
  _server.on(
      ...
      [](AsyncWebServerRequest* request) {
        ...
        auto* response = request->beginResponse(LittleFS, path, "text/plain; charset=utf-8", false);
        response->addHeader("Cache-Control", "no-store");
        request->send(response);
      });
  ...
  _server.on(
      ...
      [](AsyncWebServerRequest* request) {
        ...
        if (dir && dir.isDirectory()) {
          ...
          while (item) {
            const String itemPath = String(item.name());
            String name = itemPath;
            const int slash = name.lastIndexOf('/');
            if (slash >= 0) name = name.substring(slash + 1);
            JsonObject out = entries.add<JsonObject>();
            out["name"] = name;
            out["path"] = itemPath.startsWith("/") ? itemPath : (path == "/" ? "/" + name : path + "/" + name);
            out["type"] = item.isDirectory() ? "directory" : "file";
            out["size"] = item.isDirectory() ? 0 : item.size();
  ...
  _server.on(
      ...
      [](AsyncWebServerRequest* request) {
        ...
        if (!safePath(path) || path == LAYOUT_PATH || path == "/state/runtime-state.json") {
        ...
        target.close();
        const bool ok = isDirectory ? LittleFS.rmdir(path) : LittleFS.remove(path);
        request->send(ok ? 200 : (isDirectory ? 409 : 404), "application/json", ok ? "{\"ok\":true}" : "{\"ok\":false}");
      });
  ...
  _server.onNotFound(
      [](AsyncWebServerRequest* request) {
        if (request->url().startsWith("/api/")) {
          request->send(404, "application/json", "{\"ok\":false,\"message\":\"API route not found\"}");
          return;
        }
        const String url = request->url();
        if (LittleFS.exists(url) || LittleFS.exists(url + ".gz")) {
          sendFsFile(request, url);
          return;
        }
        sendFsFile(request, "/index.html");
      });
